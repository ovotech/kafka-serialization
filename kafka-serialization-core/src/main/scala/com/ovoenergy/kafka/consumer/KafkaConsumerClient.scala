...
import org.apache.kafka.clients.consumer.{KafkaConsumer => JavaKafkaConsumer, _}
...
private[consumer] final class KafkaConsumerClient(config: ConsumerConfig, consumerFactory: () => Consumer[Try[Key], Try[Envelope]]) extends Actor with KafkaUtils {
  ...
  private def consuming(records: ConsumerRecords[Try[Key], Try[Envelope]]): Receive = {
    ...
    subscribing orElse failing orElse {
      case _: Protocol.Poll =>
      case Protocol.Done =>
        // Async commit callback escapes from actor single thread execution.
        // Actor must not change the behaviour in the callback.
        consumer.asyncCommit { case (offsets, thrown) =>
          Option(thrown) match {
            case Some(error) => log.error(error, "Committing offset failed!")
            case None => log.debug(s"Offset commit performed, offsets: [$offsets].")
          }
        }.pipeTo(self)
